...
from plaidrl.torch.smac.agent import MakeSMACAgentDeterministic
...
class SMACInPlacePathSampler(object):
    ...
    def obtain_samples(
        self, deterministic=False, max_trajs=np.inf, max_samples=np.inf, **kwargs
    ):
        """
        ...
        policy = (
            MakeSMACAgentDeterministic(self.policy) if deterministic else self.policy
        )
        paths = []
        n_steps_total = 0
        n_trajs = 0
        while n_steps_total < max_samples and n_trajs < max_trajs:
            path = rollout(
                self.env, policy, max_path_length=self.max_path_length, **kwargs
            )
...
def rollout(
    ...
    for path_length in range(max_path_length):
        ...
        if (
            update_posterior_period != 0
            and path_length % update_posterior_period == 0
...
def rollout_multiple(
    *args, num_repeats=1, initial_context=None, accum_context=True, **kwargs
):
    """
    ...
    last_path = rollout(
        *args, accum_context=accum_context, initial_context=initial_context, **kwargs
    )
    paths = [last_path]
    for i in range(num_repeats - 1):
        ...
        new_path = rollout(
            *args, initial_context=initial_context, accum_context=True, **kwargs
        )
