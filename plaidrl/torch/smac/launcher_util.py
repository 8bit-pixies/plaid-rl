...
from plaidrl.envs.pearl_envs import AntDirEnv, HalfCheetahVelEnv
from plaidrl.torch.sac.policies import GaussianPolicy, TanhGaussianPolicy
...
def relabel_data(source_data, task, env):
    ...
    if isinstance(env, AntDirEnv):
        ...
        new_forward_reward = np.dot((torso_velocity[..., :2] / env.dt), direct).reshape(
            -1, 1
        )
    ...
    for source_task_idx in algo.replay_buffer.task_buffers:
        ...
        if num_tasks_to_relabel == "all":
            ...
            target_tasks_to_relabel = np.random.choice(
                list(algo.replay_buffer.task_buffers.keys()), num_tasks_to_relabel
            )
...
def load_macaw_buffer_onto_algo(
    ...
    task_idx_to_enc_path = get_task_idx_to_path(
        base_dir, prefix="macaw_enc_replay_buffer"
    )
...
class EvalPearl(object):
    ...
    def __call__(self):
        ...
        for name, indices in [
            ...
            ) = self.algorithm._do_eval(indices, -1)
            results["eval/adaptation/{}/final_returns Mean".format(name)] = np.mean(
                final_returns
            )
            results["eval/adaptation/{}/all_returns Mean".format(name)] = np.mean(
                online_returns
            )
    ...
    def _get_init_from_buffer_path(self, idx):
        if self.algorithm.use_meta_learning_buffer:
            init_context = self.algorithm.meta_replay_buffer._sample_contexts(
                [idx], self.algorithm.embedding_batch_size
            )
        else:
