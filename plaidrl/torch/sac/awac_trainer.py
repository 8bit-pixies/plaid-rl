...
from plaidrl.util.ml_util import ConstantSchedule, PiecewiseLinearSchedule
...
class AWACTrainer(TorchTrainer):
    ...
    def pretrain_policy_with_bc(
        ...
        for i in range(steps):
            ...
            if i % self.pretraining_logging_period == 0:
                ...
                pickle.dump(
                    self.policy,
                    open(logger.get_snapshot_dir() + "/bc_%s.pkl" % label, "wb"),
                )
                prev_time = time.time()
    ...
    def pretrain_q_with_bc_data(self):
        ...
        logger.add_tabular_output("pretrain_q.csv", relative_to_snapshot_dir=True)
