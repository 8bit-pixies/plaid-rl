...
from plaidrl.envs.pearl_envs.rand_param_envs.base import RandomEnv
...
class PR2Env(RandomEnv, utils.EzPickle):
    ...
    def __init__(self, log_scale_limit=1.0):
        self.viewer = None
        ...
        return np.concatenate(
            [
                ...
                self.model.data.qvel.flat[
                    :7
                ],  # Do not include the velocity of the target (should be 0).
    ...
    def reset_model(self):
        ...
        qpos[-3:] = goal
        qpos[:7] += self.np_random.uniform(low=-0.005, high=0.005, size=7)
        qvel[:7] += self.np_random.uniform(low=-0.005, high=0.005, size=7)
