...
class SmacTrainer(TorchTrainer):
    def __init__(
        ...
        self.train_reward_pred_in_unsupervised_phase = (
            train_reward_pred_in_unsupervised_phase
        )
        ...
        if self.use_automatic_beta_tuning:
            ...
            if beta_schedule_kwargs is None:
                ...
                schedule_class = beta_schedule_kwargs.pop(
                    "schedule_class", ml_util.PiecewiseLinearSchedule
                )
    ...
    def train_reparam_weight(self):
        if (
            self.reparam_weight_schedule_kwargs is not None
            and self.reparam_weight_schedule is None
        ):
        ...
        if self.reparam_weight_schedule is None:
            ...
            return self.reparam_weight_schedule.get_value(self._n_train_steps_total)
    ...
    def train_from_torch(self, batch):
        ...
        q_target = rewards_flat + (1.0 - terms_flat) * self.discount * target_q_values
        ...
        if self.use_reparam_update:
            policy_loss = (
                policy_loss + self.train_reparam_weight * (-q_new_actions).mean()
            )
        ...
        if self._need_to_update_eval_statistics:
            ...
            self.eval_statistics["task_embedding/kl_divergence"] = ptu.get_numpy(kl_div)
            self.eval_statistics["task_embedding/kl_loss"] = ptu.get_numpy(kl_loss)
            self.eval_statistics[
                "task_embedding/reward_prediction_loss"
            ] = ptu.get_numpy(reward_prediction_loss)
            self.eval_statistics["task_embedding/context_loss"] = ptu.get_numpy(
                context_loss
            )
            self.eval_statistics.update(
                create_stats_ordered_dict(
            ...
            self.eval_statistics["num_gradient_steps"] = self._num_gradient_steps
