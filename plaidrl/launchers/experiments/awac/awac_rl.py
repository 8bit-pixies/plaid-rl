...
# import roboverse
# from plaidrl.data_management.awr_env_replay_buffer import AWREnvReplayBuffer
from plaidrl.data_management.env_replay_buffer import EnvReplayBuffer
from plaidrl.data_management.split_buffer import SplitReplayBuffer
from plaidrl.demos.source.hdf5_path_loader import HDF5PathLoader
from plaidrl.demos.source.mdp_path_loader import MDPPathLoader

# from plaidrl.envs.images import Renderer, InsertImageEnv, EnvRenderer
from plaidrl.envs.make_env import make
from plaidrl.envs.wrappers import (
    NormalizedBoxEnv,
    RewardWrapperEnv,
    StackObservationEnv,
)
from plaidrl.exploration_strategies.base import PolicyWrappedWithExplorationStrategy
from plaidrl.exploration_strategies.gaussian_and_epsilon_strategy import (
    GaussianAndEpsilonStrategy,
)
from plaidrl.exploration_strategies.ou_strategy import OUStrategy
from plaidrl.samplers.data_collector import MdpPathCollector, ObsDictPathCollector
...
# from plaidrl.visualization.video import save_paths, VideoSaveFunction
...
def experiment(variant):
    ...
    if variant.get("use_validation_buffer", False):
        ...
        replay_buffer = SplitReplayBuffer(
            train_replay_buffer, validation_replay_buffer, 0.9
        )
    ...
    if variant.get("save_video", False):
        if variant.get("presampled_goals", None):
            variant["image_env_kwargs"]["presampled_goals"] = load_local_or_remote_file(
                variant["presampled_goals"]
            ).item()
        ...
        image_eval_env = ImageEnv(
            GymToMultiEnv(eval_env), **variant["image_env_kwargs"]
        )
