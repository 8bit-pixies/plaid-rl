...
from plaidrl.envs.env_utils import get_dim
...
class MetaLearningReplayBuffer(object):
    def __init__(
        ...
        self.sample_buffer_in_proportion_to_size = sample_buffer_in_proportion_to_size

    def create_buffer(self, size=None):
        if size is None:
            size = self.mini_buffer_max_size
        return plaidrlSimpleReplayBuffer(
    ...
    def sample_meta_batch(self, meta_batch_size, rl_batch_size, embedding_batch_size):
        ...
        if self.sample_buffer_in_proportion_to_size:
            sizes = np.array(
                [buffer.num_steps_can_sample() for buffer in self.task_buffers]
            )
