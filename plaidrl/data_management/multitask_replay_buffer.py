...
from plaidrl.data_management.replay_buffer import ReplayBuffer
from plaidrl.data_management.simple_replay_buffer import (
    SimpleReplayBuffer as plaidrlSimpleReplayBuffer,
)
...
class MultiTaskReplayBuffer(object):
    ...
    def create_new_task_buffer(self, task_idx):
        ...
        new_task_buffer = plaidrlSimpleReplayBuffer(
    ...
    def sample_context(self, indices, batch_size):
        ...
        if not hasattr(indices, "__iter__"):
            indices = [indices]
        batches = [
            self.random_batch(idx, batch_size=batch_size, sequence=False)
            for idx in indices
        ]
...
class SimpleReplayBuffer(ReplayBuffer):
    ...
    def copy_data(self, other_buffer: "SimpleReplayBuffer"):
        ...
        self._observations[start_i:end_i] = other_buffer._observations[
            : other_buffer._top
        ].copy()
        self._actions[start_i:end_i] = other_buffer._actions[: other_buffer._top].copy()
        self._rewards[start_i:end_i] = other_buffer._rewards[: other_buffer._top].copy()
        self._terminals[start_i:end_i] = other_buffer._terminals[
            : other_buffer._top
        ].copy()
        self._next_obs[start_i:end_i] = other_buffer._next_obs[
            : other_buffer._top
        ].copy()
        self._sparse_rewards[start_i:end_i] = other_buffer._sparse_rewards[
            : other_buffer._top
        ].copy()
