...
from plaidrl.core import eval_util, logger
from plaidrl.core.logging import append_log
from plaidrl.data_management.meta_learning_replay_buffer import MetaLearningReplayBuffer
from plaidrl.data_management.multitask_replay_buffer import MultiTaskReplayBuffer
from plaidrl.data_management.path_builder import PathBuilder
from plaidrl.torch import pytorch_util as ptu
from plaidrl.torch.smac.sampler import SMACInPlacePathSampler
...
class MetaRLAlgorithm(metaclass=abc.ABCMeta):
    def __init__(
        ...
        """
        self.debug_enc_buffer_matches_rl_buffer = debug_enc_buffer_matches_rl_buffer
        self.logging_period = logging_period
        self.train_agent_in_unsupervised_phase = train_agent_in_unsupervised_phase
        ...
        self.num_initial_steps_self_generated_tasks = (
            num_initial_steps_self_generated_tasks
        )
        ...
        self.train_encoder_decoder_in_unsupervised_phase = (
            train_encoder_decoder_in_unsupervised_phase
        )
        ...
        self.save_extra_manual_beginning_epoch_list = (
            save_extra_manual_beginning_epoch_list
        )
        ...
        if num_iterations_with_reward_supervision is None:
            num_iterations_with_reward_supervision = np.inf
        self.num_iterations_with_reward_supervision = (
            num_iterations_with_reward_supervision
        )
        ...
        self.meta_replay_buffer = MetaLearningReplayBuffer(
            ...
            mini_buffer_max_size=self.max_path_length
            + max(
    ...
    def train(self):
        ...
        for it_ in gt.timed_for(
            ...
            clear_encoder_buffer = (
                update_encoder_buffer and self.clear_encoder_buffer_before_every_update
            ) and not self.use_rl_buffer_for_enc_buffer
    ...
    def pretrain(self):
        ...
        num_existing_tasks = len(self.offline_train_task_indices) + len(
    ...
    def collect_exploration_data(
        self,
        num_samples,
        resample_latent_period,
        update_posterior_period,
        task_idx,
                ...
                if use_predicted_reward:
                    if self.use_meta_learning_buffer:
                        ...
                        initial_reward_context = (
                            self._reward_decoder_buffer.sample_context(
                                task_idx, self.embedding_batch_size
                            )
                        )
    ...
    def _try_to_eval(self, epoch):
        ...
        if self._can_evaluate():
            ...
            logger.record_tabular(
                "in_unsupervised_model", float(self.in_unsupervised_phase)
            )
    ...
    def evaluate(self, epoch):
        ...
        if self.dump_eval_paths:
            ...
            logger.save_extra_data(
                prior_paths, file_name="eval_trajectories/prior-epoch{}".format(epoch)
            )
        ### train tasks
        if self._num_tasks_to_eval_on >= len(self.train_task_indices):
            ...
            indices = np.random.choice(
                self.offline_train_task_indices, self._num_tasks_to_eval_on
            )
        ...
        if len(self.fake_task_idx_to_z) > 0:
            ...
            (
                ...
                self_generated_online_returns,
                _,
            ) = self._do_eval(self_generated_indices, epoch)
        ...
        try:
            ...
            pass
        logger.save_extra_data(
            avg_train_online_return, file_name="online-train-epoch{}".format(epoch)
        )
        logger.save_extra_data(
            avg_test_online_return, file_name="online-test-epoch{}".format(epoch)
        )
    ...
    def _get_z_distribution_log(self, idx_to_final_context):
        ...
        offline_context1 = self._reward_decoder_buffer.sample_context(
            task_indices, self.embedding_batch_size
        )
        ...
        context_distribs1 = [
            Normal(m, s) for m, s in zip(context_distrib1.mean, context_distrib1.stddev)
        ]
        context_distribs2 = [
            Normal(m, s) for m, s in zip(context_distrib2.mean, context_distrib2.stddev)
        ]
        (
        ...
        logs["two_offline_z_posteriors/within_task_kl"] = within_task_kl_2samples
        logs["two_offline_z_posteriors/between_task_kl"] = between_task_kl_2samples
        offline_z_posterior_kl_prior = kl_divergence(
            context_distrib1, self.agent.latent_prior
        ).sum(dim=1)
        logs.update(
            eval_util.create_stats_ordered_dict(
        ...
        logs["offline_vs_online_z_posterior/within_task_kl"] = within_task_kl_off_on
        logs["offline_vs_online_z_posterior/between_task_kl"] = between_task_kl_off_on
    ...
    def _compute_within_and_between_task_kl(self, context_distribs1, context_distribs2):
